...
router.post('/api/order', async (req, res) => {
  ...
  try {
    ...
    for (const order of orders) {
      console.log(order)
      const { itemName, rate, qty, tableNo, itemCode, userID, currentTableType, amount, bookingDate,bookingTime } = order;
      const Kot_Type = 'KOT'
      ...
      request.input(`bookingTime_${paramNamePrefix}`,sql.VarChar,bookingTime);
